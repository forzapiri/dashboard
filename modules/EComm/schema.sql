...
INSERT INTO `config_options` (`id`, `module`, `name`, `description`, `type`, `value`, `sort`, `editable`) VALUES 
(NULL, 'EComm', 'CurrencySign', 'Sets the currency sign to be used in the EComm module', 'string', '$', '10', '1'), 
(NULL, 'EComm', 'productPlugins', 'A list of ECommProduct plugins to be loaded', 'list', 'Versions,AlternativePics,Testemonials', '10', '0'), 
(NULL, 'EComm', 'paymentPlugins', 'Set payment plugins to be loaded', 'list', 'Paypal', '10', '0'), 
(NULL, 'EComm', 'shippingPlugins', 'Set shipping plugins to be loaded', 'list', '', '10', '0'), 
(NULL, 'EComm', 'shippingEnabled', 'Enable/disable need of shipping info', 'int', '1', '10', '0'),
(NULL, 'EComm', 'PaypalBusinessEmailAddress', '', 'enum(jacob@norex.ca)', 'jacob@norex.ca', '10', '0'), 
(NULL, 'EComm', 'PaypalHostName', '', 'enum(www.sandbox.paypal.com,www.paypal.com)', 'www.sandbox.paypal.com', '10', '0'),
(NULL, 'EComm', 'Currency', '', 'enum(CAD,USD,EUR,GBP)', 'CAD', '10', '0');
...
VALUES (
'Module_EComm', 'Display.tpl', '{assign var="returnURL" value="/Store"} {if $group} {assign var="secId" value=$group->getId()} {assign var="returnURL" value="/Store/$section/$secId"} {/if} {if $group && $section=="Category"} {include file="Category.tpl"} {/if} {if $group && $section=="ProductType"} {include file="ProductType.tpl"} {/if} {if $group && $section=="Supplier"} {include file="Supplier.tpl"} {/if} {if $results} {include file="DisplayGroup.tpl"} {/if} {if $products} {include file="SearchResults.tpl"} {/if} ',
CURRENT_TIMESTAMP , NULL , NULL
), ( 
'Module_EComm', 'Category.tpl', '<h2>Category: {$group->getName()}</h2> <div class="CellInfo">  <div class="CellInfoImage">  {*{if $group->getImage()} <img border=0 src="/images/image.php?id={$group->getImage()}&cliph=140">  {else} <img border=0 src="/modules/EComm/images/no_image.gif">  {/if}*} </div>  <div class="CellInfoDetails">  {$group->getDetails()} </div> </div> {$group->displayBreadCrumb()} <br style="clear:both;"/>',
CURRENT_TIMESTAMP , NULL , NULL
), (
'Module_EComm', 'SearchResults.tpl', '<h2>Products:</h2>
{if !$products}
...
{/if}
', CURRENT_TIMESTAMP, NULL, NULL), (
'Module_EComm', 'Product.tpl', '{if $msg}<center><h2>{$msg}</h2></center>{/if}
{if $product}
...
{/if}', CURRENT_TIMESTAMP, NULL, NULL), (
'Module_EComm', 'DisplayCart.tpl', '{include file="CartDetails.tpl" page="Display"}
{if $loginFail}
...
{/if}
{$user_form->display()}', CURRENT_TIMESTAMP, NULL, NULL);
