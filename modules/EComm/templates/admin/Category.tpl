...
<h3>Here, you can manage the categories</h3>
<a href="/admin/EComm&section=Category&action=addedit">Add a new category</a>

<table class="adminList" style="clear: both; float: left;" border="0" cellpadding="0" cellspacing="0">
	<tbody><tr>
		<th valign="center">Title</th>
		<th valign="center">Parent</th>
	...
	<tr class="{cycle values="row1,row2"}">
		<td>{$obj->getName()}</td>
		<td>{$obj->getParentCategory()|get_db_value:ecomm_category:id:name}</td>
		...
		<td>
			<form method="POST" action="/admin/EComm" style="float: left;">
				<input type="hidden" name="category_id" value="{$obj->getId()}" />
